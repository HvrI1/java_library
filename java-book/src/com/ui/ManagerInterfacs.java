...
public class ManagerInterfacs {
    JFrame jf = new JFrame("王明图书馆，欢迎您");
    final int  WIDTH = 1000;
    final int HEIGHT = 600;

    public ManagerInterfacs() {
    }
...
//            jf.setIconImage(ImageIO.read(new File("src/img/logo.png")));
        ...
        DefaultMutableTreeNode borrowManage = new DefaultMutableTreeNode("数量管理");
...
//得到当前选中的结点对象
            ...
            if (borrowManage.equals(lastPathComponent)) {
                try {
                    sp.setRightComponent(new RootUpdateManageComponent(jf));
                    sp.setDividerLocation(150);
                } catch (SQLException | ClassNotFoundException throwables) {
                    throwables.printStackTrace();
                }
